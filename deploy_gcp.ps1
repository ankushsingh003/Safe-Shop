...
if (-not $PROJECT_ID) {
    Write-Host "❌ No GCP project is currently set." -ForegroundColor Red
    Write-Host "💡 Please run: gcloud config set project <PROJECT-ID>" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "📦 Step 1: Building Docker image using Cloud Build..." -ForegroundColor Yellow
...
Write-Host "🔗 Your API is now live at the Cloud Run URL shown above!" -ForegroundColor Green
